...
const ShowGroupMembers=(props)=>{
    ...
    const removeMemberHandler=()=>{
        axios.delete('http://localhost:3000/delete-member',{
    ...
    const makeAdminHandler=()=>{
        axios.put('http://localhost:3000/make-admin',{
            userId:props.id,groupId:groupId
        },{ headers:{"Authorization":authToken}
        })
        .then(()=>{
    ...
    return (
        <div className="showresult">
         ...
         {isAdmin && props.admin==false && (<div><button className='button3' onClick={makeAdminHandler}>Make Admin</button></div>)}
         {isAdmin && props.admin==false && (<div><button className='button3' onClick={removeMemberHandler}>x</button></div>)}
         </div>
    )
}
